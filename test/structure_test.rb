require_relative "test_helper"

class Norma19::StructureTest < Test::Unit::TestCase
  def setup
    opts = JSON.parse( File.read( "#{FIXTURES}/opts.json" ), :symbolize_names => true)
    payers = JSON.parse( File.read( "#{FIXTURES}/payers.json" ), :symbolize_names => true )

    Delorean.time_travel_to( "2001-02-03" ) do
      @structure = Norma19::Structure.new( opts, payers )
    end
  end

  def test_initialize
    Norma19::Structure.any_instance.expects( :generate_extra_opts ).returns( { :key_extra => "value_extra" } )
    opts = { :key_1 => "value_1" }
    payers = "payers"

    structure = Norma19::Structure.new( opts, payers )

    assert_equal( "value_1", structure.opts[:key_1] )
    assert_equal( "value_extra", structure.opts[:key_extra] )
    assert_equal( "payers", structure.payers )
  end

  def test_all_lines_total_sizes
    assert_equal( 162, Norma19::Structure::HEAD_1.map{ |e| e[:size] }.reduce(:+) )
    assert_equal( 162, Norma19::Structure::HEAD_2.map{ |e| e[:size] }.reduce(:+) )
    assert_equal( 162, Norma19::Structure::PAYER_1.map{ |e| e[:size] }.reduce(:+) )
    assert_equal( 162, Norma19::Structure::PAYER_2.map{ |e| e[:size] }.reduce(:+) )
    assert_equal( 162, Norma19::Structure::FOOTER_1.map{ |e| e[:size] }.reduce(:+) )
    assert_equal( 162, Norma19::Structure::FOOTER_2.map{ |e| e[:size] }.reduce(:+) )
  end

  def test_generate_extra_opts
    Delorean.time_travel_to( "2001-02-03" ) do
      extra_opts = @structure.generate_extra_opts
      assert_equal( "2001-02-03", extra_opts[:file_created_at] )
      assert_equal( 3, extra_opts[:total_payers] )
      assert_equal( 7, extra_opts[:total_records_collector] )
      assert_equal( 9, extra_opts[:total_records] )
      assert_equal( 200011.01, extra_opts[:total_amount] )
    end
  end

  def test_generate_head_1
    assert_equal( read_spaced_fixture( "head_1.txt" ), @structure.generate_head_1 )
  end

  def test_generate_head_2
    assert_equal( read_spaced_fixture( "head_2.txt" ), @structure.generate_head_2 )
  end

  # def test_generate_footer_1
  #   assert_equal( read_spaced_fixture( "footer_1.txt" ), @structure.generate_head_2 )
  # end
end